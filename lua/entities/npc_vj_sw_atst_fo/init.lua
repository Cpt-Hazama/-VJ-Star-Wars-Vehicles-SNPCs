...
-----------------------------------------------*/
ENT.Model = {"models/cpthazama/starwars/atst.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 500
...
ENT.SoundTbl_FootStep = {"cpthazama/atst_step.mp3"}
ENT.SoundTbl_Move = {"cpthazama/atst_move.mp3"}
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnAcceptInput(key,activator,caller,data)
	if key == "event_emit step" then
		...
		util.ScreenShake(self:GetPos(), 100, 200, 1, 1300)
	end
	if key == "event_emit move" then
		VJ_EmitSound(self,self.SoundTbl_Move,self.FootStepSoundLevel,self:VJ_DecideSoundPitch(self.FootStepPitch1,self.FootStepPitch2))
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:RangeAttackCode_GetShootPosTwin(TheProjectile)
	return (self:GetEnemy():GetPos() +self:GetEnemy():OBBCenter() -(self:GetAttachment(self:LookupAttachment("twin_cannon")).Pos)):GetNormal() *150000
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:RangeAttackCode_GetShootPos(TheProjectile)
	return (self:GetEnemy():GetPos() +self:GetEnemy():OBBCenter() -(self:GetAttachment(self:LookupAttachment("grenade_cannon")).Pos)):GetNormal() *800 +self:GetUp() *650
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnInitialize()
	self:SetCollisionBounds(Vector(60,60,330), Vector(-60,-60,0))
	self.NextGunAttackT = CurTime()
	self.NextTwinGunAttackT = CurTime() +2
	self.NextGrenadeAttackT = CurTime() +6
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:TwinCannon(enemy,ovtime)
	if ovtime == nil then
		ovtime = math.Rand(2,4)
	end
	if CurTime() > self.NextTwinGunAttackT then
		local proj = ents.Create("obj_vj_sw_twinblast")
		proj:SetPos(self:GetAttachment(self:LookupAttachment("twin_cannon")).Pos)
		...
		if (phys:IsValid()) then
			phys:Wake()
			phys:SetVelocity(self:RangeAttackCode_GetShootPosTwin(proj))
		end
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:GrenadeCannon(enemy,ovtime)
	if ovtime == nil then
		ovtime = math.Rand(5,12)
	end
	if CurTime() > self.NextGrenadeAttackT then
		local proj = ents.Create("obj_vj_sw_concgren")
		proj:SetPos(self:GetAttachment(self:LookupAttachment("grenade_cannon")).Pos)
		proj:SetAngles((enemy:GetPos() -proj:GetPos()):Angle())
		proj:Spawn()
		proj:Activate()
		proj:SetOwner(self)
		...
		VJ_EmitSound(self,{"weapons/mortar/mortar_fire1.wav"},100,self:VJ_DecideSoundPitch(100,110))
		timer.Simple(0.2,function()
			if IsValid(self) && IsValid(enemy) then
				local proj = ents.Create("obj_vj_sw_concgren")
				proj:SetPos(self:GetAttachment(self:LookupAttachment("grenade_cannon")).Pos)
		...
		self.NextGrenadeAttackT = CurTime() +ovtime
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
